...
### 3.2.1 Phân tích chuyên sâu (Technical Deep Dive)

Để hiểu rõ hơn vì sao kiến trúc này được đánh giá là "Production-Ready", dưới đây là phân tích luồng hoạt động kỹ thuật của 7 thành phần:

1. **User Interface (Giao diện người dùng):** Ứng dụng không chạy trên máy chủ ảo truyền thống (EC2). Toàn bộ mã nguồn React được build tĩnh và lưu trên Amazon S3. AWS CloudFront (CDN) phân phối cache trên toàn cầu giúp tốc độ tải trang cực mượt. Đặc biệt, việc bật **AWS WAF** trên CloudFront tạo ra khiên chắn Layer 7, bẻ gãy mọi nỗ lực DDoS hoặc SQL Injection trước khi chúng kịp chạm tới hệ thống.
2. **Application Compute (Xử lý cốt lõi):** Đưa FastAPI lên Lambda là một thách thức vì Lambda giới hạn gói code 250MB, trong khi các thư viện PDF rất nặng. Nhóm giải quyết bằng cách đóng gói ứng dụng thành **Docker Container Image** lưu tại Amazon ECR. Bằng cách dùng thư viện `Mangum`, Lambda phân giải hoàn hảo các request HTTP từ API Gateway thành chuẩn ASGI cho FastAPI. Nhờ cơ chế Scale-to-Zero, hệ thống tiêu tốn $0 khi không có traffic.
3. **AI / ML Feature (Trí tuệ nhân tạo):** Xử lý hàng nghìn dòng sao kê bằng AI rất tốn chi phí Token. Nhóm thiết kế một **Kiến trúc Hybrid**: Bộ lọc Rule-based (Regex) tại Lambda sẽ quét qua file trước để phân loại các giao dịch quen thuộc. Chỉ những giao dịch "khó" mới được đưa vào `ThreadPoolExecutor` để gọi xử lý song song lên Bedrock (Amazon Nova Lite 2). Kiến trúc này giảm tới 80% tải cho LLM, tối ưu cực tốc độ và chi phí.
4. **Data Persistence (Lưu trữ dữ liệu):** Dữ liệu tài chính đòi hỏi độ an toàn tuyệt đối. Nhóm quyết định nâng cấp lên kiến trúc **RDS PostgreSQL Multi-AZ** (1 Primary + 1 Read Replica). Để cân bằng chi phí, nhóm khôn khéo chọn dòng chip ARM t4g.micro đời mới. Khi đối mặt bài toán bùng nổ kết nối (Connection Spike), do bị cấm tạo RDS Proxy ở Free Tier, nhóm đã sáng tạo dùng **SQS làm Buffer tĩnh**. Lambda Worker chỉ kéo một lượng message vừa đủ khả năng chịu đựng của DB, triệt tiêu hoàn toàn nguy cơ sập Database.
5. **Object Storage (Lưu trữ tệp lớn):** Tải file PDF sao kê hàng chục MB qua API Gateway sẽ bị lỗi do giới hạn payload 10MB. Nhóm thiết kế luồng **S3 Presigned URL**. Frontend gọi Lambda xin một "tấm vé thông hành" có thời hạn. Trình duyệt dùng vé này upload file thẳng lên S3 với tốc độ tối đa, giảm tải băng thông 100% cho Backend.
6. **Network Foundation (Hạ tầng mạng):** Bảo mật dữ liệu là ưu tiên số 1. RDS được giấu trong Private Subnet, cách ly hoàn toàn khỏi Internet. Để Lambda trong Private Subnet gọi được API của Amazon Bedrock mà không cần xài NAT Gateway đắt đỏ, nhóm đã thiết lập **VPC Interface Endpoint (PrivateLink)**. Toàn bộ dữ liệu AI chạy nội bộ trên mạng xương sống (Backbone) của AWS.
7. **Identity & Access (Định danh & Truy cập):** Hệ thống dùng Amazon Cognito để quản lý user và cấp phát JSON Web Token (JWT). Tại cửa ngõ API Gateway được gắn JWT Authorizer. Bất kỳ request nào không có token hợp lệ đều bị từ chối với lỗi 401 Unauthorized ngay lập tức. Nhờ lớp **Edge Security** này, phần Compute Lambda bên trong hoàn toàn không bị đánh thức bởi request rác.
